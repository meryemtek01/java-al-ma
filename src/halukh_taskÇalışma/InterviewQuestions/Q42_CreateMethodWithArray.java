...
//-5 boyutunda bir dizi oluşturun, 0'dan 10'a kadar rastgele değerler atayın
        Integer arr [] = new Integer[5];//5 elaman alacak boş array tanımandı
        Random rnd=new Random();
        for (int i = 0; i <=5 ; i++) {
            arr[i] = rnd.nextInt(10);//0 10 random sayıları arraya atandı
...
maxArry(arr);
    ...
    private static void maxArry(Integer[] arr) {
        int maxValue =arr[0];
        for (int avuc:arr) {
    ...
    private static void sumArry(Integer[] arr) {
        int toplam=0;
        for (int avuc:arr) {
           toplam+=avuc;
    ...
    private static void printArry(Integer[] arr) {
